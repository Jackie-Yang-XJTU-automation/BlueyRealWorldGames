...
import { TodayRecommendationPanel } from '../components/TodayRecommendationPanel'
import { useFavorites } from '../hooks/useFavorites'
import { useTodayRecommendations } from '../hooks/useTodayRecommendations'
import type { FilterOptions, Game } from '../types/game'
import type { PlayLaunchSource, TodayRecommendation } from '../types/playExperience'
...
const SCENARIO_SHORTCUTS: ScenarioShortcut[] = [
  {
    ...
    label: '家里安静玩',
    hint: '客厅可开局',
    match: game => ['magic-xylophone', 'hospital', 'claw-machine', 'shops'].includes(game.id) || (game.location === 'indoor' && game.energy === 1),
  },
  {
    id: 'burn-energy',
    emoji: '⚡',
  ...
  },
  {
    ...
    label: '爸妈一起演',
    hint: '角色扮演',
    match: game => ['daddy-robot', 'hospital', 'bbq', 'claw-machine', 'spy-game', 'shops'].includes(game.id) || game.type === 'roleplay',
  },
  {
    id: 'quick-five',
    emoji: '⏱️',
  ...
  },
  {
    ...
    label: '不用材料',
    hint: '马上开始',
    match: game => game.materials.length === 0 || ['daddy-robot', 'shadowlands', 'shops'].includes(game.id),
  },
  {
    id: 'bedtime',
    emoji: '🌙',
    label: '睡前可玩',
    hint: '低刺激收尾',
    match: game => ['hospital', 'bbq', 'magic-xylophone', 'shops'].includes(game.id) || (game.energy === 1 && game.type !== 'active'),
  },
]
...
export function HomePage() {
  ...
  const {
    ...
    summary: playSummary,
    setContext: setRecommendationContext,
  ...
  const spotlightGame = useMemo(() => {
    ...
    return primaryRecommendation?.game ?? playableGames[new Date().getDate() % playableGames.length]
  }, [primaryRecommendation])
  ...
  const handleOpenGame = useCallback((game: Game, source: PlayLaunchSource = 'card', note?: string) => {
    const playable = getPlayableGame(game.id)
    markLaunched(game, source, note)
    navigate(playable?.route ?? `/game/${game.id}`)
  }, [markLaunched, navigate])

  const handleOpenRecommendation = useCallback((recommendation: TodayRecommendation) => {
    handleOpenGame(recommendation.game, 'today', recommendation.rule.title)
  }, [handleOpenGame])
  ...
  return (
    <div>
      <EpisodeHero
        ...
        onOpenGame={(game) => handleOpenGame(game, 'today', '首页首屏')}
      ...
      <TodayRecommendationPanel
        ...
        context={recommendationContext}
        launchesToday={playSummary.launchesToday}
        onContextChange={setRecommendationContext}
      ...
      <section aria-label="按周末场景选游戏" className="mb-5">
        <div className="mb-2 flex items-end justify-between gap-3 px-1">
          <div>
            ...
            <p className="text-xs font-extrabold text-[#5C728D]">不记剧情也可以直接开演</p>
          </div>
          <span className="rounded-full bg-white/80 px-2.5 py-1 text-[10px] font-black text-[#5C728D] shadow-[0_2px_0_rgba(174,224,250,0.45)]">
            家长选
          </span>
        </div>
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
          {scenarioPicks.map(({ shortcut, game }) => {
            ...
            <button
              key={shortcut.id}
              type="button"
              onClick={() => handleOpenGame(game, 'scenario', shortcut.label)}
            ...
            >
              ...
              <span className="block pl-11 text-[11px] font-extrabold leading-snug text-[#5C728D]">{shortcut.hint}</span>
              <span className="mt-1 block truncate pl-11 text-[10px] font-black text-[#5C728D]">
      ...
      <div className="mb-5 rounded-[28px] border-2 border-white/90 bg-[#FDFBF7]/86 p-3.5 shadow-[0_6px_0_rgba(174,224,250,0.42),0_10px_24px_rgba(44,67,100,0.08)]">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="btv-display text-[13px] uppercase text-[#5C728D]">想找别的玩法？</h2>
            <p className="text-[11px] font-extrabold text-[#5C728D]">
          ...
          >
            {showFilters ? '收起' : `${filteredGames.length} 个`}
          </button>
        </div>
        {showFilters && (
          <div className="mt-3">
      ...
      {randomGame && (
        ...
        >
          <div className="relative w-full max-w-[340px] rounded-[30px] border-[3px] border-[#F9D06B] bg-[#FDFBF7] p-5 text-center shadow-[0_18px_40px_rgba(44,67,100,0.18)] animate-jelly">
            ...
            <p className="mx-auto mb-3 max-w-[250px] text-sm font-extrabold leading-snug text-[#5C728D]">
              {getParentPlayHint(randomGame.id).kidHook}
            </p>
            <div className="mb-5 flex flex-wrap items-center justify-center gap-1.5 text-[11px] font-black text-[#5C728D]">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="min-h-12 flex-1 rounded-full bg-[#F0F4FF] px-3 py-3 text-sm font-extrabold text-[#5C728D] transition-all hover:bg-[#E3ECFD] hover:text-[#5C728D] active:scale-95"
              >
                再选一次
              </button>
              <button
                type="button"
                onClick={() => { setRandomGame(null); handleOpenGame(randomGame, 'random', '随机救场') }}
          ...
          {favoritesExpanded && (
            <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
              {favoriteGames.map(game => (
                <div
                  key={game.id}
                  onClick={() => handleOpenGame(game, 'favorite', '收藏夹')}
      ...
      <section aria-label="全部游戏">
        <h3 className="btv-display mb-3 flex items-center gap-2 text-sm uppercase text-[#5C728D]">
        ...
        {filteredGames.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-xl font-extrabold text-[#5C728D]">没有符合的游戏</p>
            <p className="text-sm text-[#5C728D] mt-1.5 font-bold">试试换个类型或场地看看？</p>
          </div>
        ) : (
            ...
            {sortedFilteredGames.map((game, i) => (
              <div key={game.id} ref={node => setCardRef(game.id, node)}>
                <GameCard
                  ...
                  onClick={() => handleOpenGame(game, 'card', '全部陪玩卡')}
                  index={i}
