            ...
            <button
              ...
              className={`shrink-0 tag-btv text-[13px] py-2.5 px-4 font-extrabold transition-all duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] active:scale-95 ${
                active
                  ...
                  : 'bg-[#E8F7FF] text-[#5C728D] shadow-[0_3px_0_rgba(174,224,250,0.50)] hover:bg-[#D7F0FF] hover:text-[#5a5a87] hover:scale-[1.03]'
              }`}
            >
              {o.label}
            </button>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button
          ...
          className={`shrink-0 tag-btv text-xs py-2.5 px-3.5 font-extrabold transition-all duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] active:scale-95 ${
            ...
            : 'bg-[#E8F7FF] text-[#5C728D] shadow-[0_3px_0_rgba(174,224,250,0.50)] hover:bg-[#D7F0FF] hover:scale-[1.03]'
          }`}
        >
        ...
        <button
          type="button"
          onClick={() => {
            const levels: FilterOptions['difficulty'][] = ['all', 1, 2, 3]
            const idx = levels.indexOf(filters.difficulty)
            onFilterChange({ ...filters, difficulty: levels[(idx + 1) % levels.length] })
          }}
          className={`shrink-0 tag-btv px-3.5 py-2.5 text-xs font-extrabold transition-all duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] active:scale-95 ${
            filters.difficulty === 1 ? 'bg-btv-green text-white shadow-[0_4px_0_rgba(76,175,80,0.22),0_8px_16px_rgba(76,175,80,0.14)] scale-105'
            : filters.difficulty === 2 ? 'bg-btv-yellow text-[#5a5a87] shadow-[0_4px_0_rgba(252,216,130,0.38),0_8px_16px_rgba(252,216,130,0.18)] scale-105'
            : filters.difficulty === 3 ? 'bg-btv-red text-white shadow-[0_4px_0_rgba(217,107,98,0.22),0_8px_16px_rgba(217,107,98,0.16)] scale-105'
            : 'bg-[#E8F7FF] text-[#5C728D] shadow-[0_3px_0_rgba(174,224,250,0.50)] hover:bg-[#D7F0FF] hover:scale-[1.03]'
          }`}
        >
          {filters.difficulty === 1 ? '⭐ 入门' : filters.difficulty === 2 ? '⭐⭐ 进阶' : filters.difficulty === 3 ? '⭐⭐⭐ 需要主持' : '⭐ 难度'}
        </button>
