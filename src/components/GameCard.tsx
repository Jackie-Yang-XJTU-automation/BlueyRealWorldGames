...
export function GameCard({ game, isFavorite, onToggleFavorite, onClick, index = 0 }: GameCardProps) {
  ...
  return (
    <div
      className="relative rounded-[28px] bg-white border-2 border-[#E3F2FD] hover:border-[#BBDEFB] shadow-[0_6px_20px_rgba(28,152,237,0.08)] cursor-pointer group overflow-hidden active:scale-[0.97] hover:-translate-y-1.5 hover:scale-[1.03] transition-all duration-400 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] animate-card-enter"
    ...
    >
      ...
      <button
        ...
        className={`absolute top-2.5 right-2.5 w-[44px] h-[44px] min-w-[44px] min-h-[44px] bg-white/90 hover:bg-white rounded-full flex items-center justify-center text-lg shadow-sm hover:scale-110 active:scale-90 transition-transform z-10 ${heartBounce ? 'animate-heart-pop' : ''}`}
      >
        {isFavorite ? '❤️' : '🤍'}
      </button>
    </div>
